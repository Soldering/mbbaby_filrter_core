...
foreach($arResult["ITEMS"] as $PID => $arItem)
{
    ...
    $this->SKU_IBLOCK_ID = 30;
	if(isset($arItem["PRICE"]))
	{
		if(strlen($arItem["VALUES"]["MIN"]["HTML_VALUE"]) && strlen($arItem["VALUES"]["MAX"]["HTML_VALUE"]))
			${$FILTER_NAME}["OFFERS"]["><CATALOG_PRICE_".$arItem["ID"]] = array($arItem["VALUES"]["MIN"]["HTML_VALUE"], $arItem["VALUES"]["MAX"]["HTML_VALUE"]);
		elseif(strlen($arItem["VALUES"]["MIN"]["HTML_VALUE"]))
			${$FILTER_NAME}["OFFERS"][">=CATALOG_PRICE_".$arItem["ID"]] = $arItem["VALUES"]["MIN"]["HTML_VALUE"];
		elseif(strlen($arItem["VALUES"]["MAX"]["HTML_VALUE"]))
			${$FILTER_NAME}["OFFERS"]["<=CATALOG_PRICE_".$arItem["ID"]] = $arItem["VALUES"]["MAX"]["HTML_VALUE"];
	}
...
if(isset($_REQUEST["ajax"]) && $_REQUEST["ajax"] === "y")
{
    ...
    if(!empty($arFilter["OFFERS"])){
        $arFilter["OFFERS"]["ACTIVE"]="Y";
        $arFilter["=ID"]=[1];
        $res = \CIBlockElement::GetList([],$arFilter["OFFERS"],false,false,["NAME","ACTIVE",'PROPERTY_140']);
        while($arRes = $res->GetNext()){
            $arFilter["=ID"][$arRes["PROPERTY_140_VALUE"]]=$arRes["PROPERTY_140_VALUE"];

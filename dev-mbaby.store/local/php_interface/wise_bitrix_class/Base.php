...
abstract class Base extends \CBitrixComponent
{
    ...
    protected function getElementList($iblockId, $products)
    {
        ...
        if(!empty($filter["OFFERS"])){
            $filter["OFFERS"]["ACTIVE"]="Y";
            $filter["=ID"]=[1];
            $res = \CIBlockElement::GetList([],$filter["OFFERS"],false,false,["NAME","ACTIVE",'PROPERTY_140']);
            while($arRes = $res->GetNext()){
                $filter["=ID"][$arRes["PROPERTY_140_VALUE"]]=$arRes["PROPERTY_140_VALUE"];
    ...
    protected function getIblockOffers($iblockId)
    {
        ...
        )
        {
            ...
            $productProperty = 'PROPERTY_140';
            $productPropertyValue = $productProperty.'_VALUE';
            if($catalog['IBLOCK_ID']==26){
                $catalog['IBLOCK_ID'] = 30;
            }
            ...
            if(!empty(${$this->arParams["FILTER_NAME"]}["OFFERS"])){
                $offersFilter = array_merge($offersFilter,${$this->arParams["FILTER_NAME"]}["OFFERS"]);
            }
            
            $offersId = array();
            $offersCount = array();
            
            unset($offersFilter['SHOW_PRICE_COUNT']);
            unset($offersFilter['ACTIVE_DATE']);

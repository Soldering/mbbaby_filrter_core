...
foreach ($arResult['ITEMS'] as $key => &$arItem_Detail)
{
	if(!empty($arItem_Detail["OFFERS"])){
		
		foreach ($arItem_Detail["OFFERS"] as $index => &$OFFER_Detail) {
			$OFFER_Detail["DETAIL_PAGE_URL"] = $arItem_Detail["DETAIL_PAGE_URL"]."?oid=".$OFFER_Detail["ID"];
		}
	}
...
if (!empty($arResult['ITEMS'])){
	...
	if ($arResult['MODULES']['catalog'])
	{
		...
		if ($boolSKU && !empty($arParams['OFFER_TREE_PROPS']) && 'TYPE_1' == $arParams['TYPE_SKU'] && $arParams['DISPLAY_TYPE'] !='table')
		{
			...
			$arNeedValues = array();
			IblockProperty::getTreePropertyValues($arSKUPropList, $arNeedValues);
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		$flag->setArResult($arItem);
		$flag->setFlag();
		$arItem = $flag->getArResult();
		...
		if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
			if ('TYPE_1' == $arParams['TYPE_SKU'] && $arParams['DISPLAY_TYPE'] !='table')
			{
				...
				foreach ($arSKUPropIDs as $propkey => $strOneCode)
				{
					...
					{
						if (intval(current($arPropSKU[$strOneCode])["NAME"]) != false) {
							Collection::sortByColumn($arPropSKU[$strOneCode], array("NAME" => array(SORT_NUMERIC))); // sort sku prop values
						} else {
							Collection::sortByColumn($arPropSKU[$strOneCode], array("NAME" => array(SORT_ASC))); // sort sku prop values
						}
				...
				foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
				{
					...
					$sPriceMatrix = '';
					if($arParams['USE_PRICE_COUNT'] == 'Y' && false)
					{
					...
					$arOneRow = array(
						...
						'PREVIEW_PICTURE_JS' => $arOffer['PREVIEW_PICTURE'],
						'PREVIEW_PICTURE_SECOND_JS' => $arOffer['PREVIEW_PICTURE_SECOND'],
						...
						'AVAILIABLE' => WsCMShop::GetQuantityArray($arOffer['CATALOG_QUANTITY']),
						...
						'LAST_SIZE'=> $arOffer["PROPERTIES"]["LAST_SIZE"]["VALUE_XML_ID"]=="Y"?"Y":"N",
					);
				...
				if($arOffer["PROPERTIES"]["LAST_SIZE"]["VALUE_XML_ID"]=="Y"){
					$arItem["LAST_SIZE"]="Y";
				}

		...
		<div class="bx_filter_section">
			<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter" id="smartfilter">
				...
				foreach($arResult["ITEMS"] as $key=>$arItem)
				{
                    ...
                    $style = "";
                    if($disabled == count($arItem["VALUES"])){
                        $style="display:none;";
                    }
                    ?>
					<div class="bx_filter_parameters_box <?=$class;?>" style="<?=$style?>" data-expanded="<?=($arItem["DISPLAY_EXPANDED"] ? $arItem["DISPLAY_EXPANDED"] : "N");?>" data-prop_code=<?=strtolower($arItem["CODE"]);?> data-property_id="<?=$arItem["ID"]?>">
						<span class="bx_filter_container_modef"></span>
									...
									<?foreach($arItem["VALUES"] as $val => $ar):?>
										...
										<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label <?=($isSize ? "nab sku" : "");?> <?=($i==$count ? "last" : "");?> <? echo $ar["DISABLED"] ? 'disabled': '' ?>" for="<? echo $ar["CONTROL_ID"] ?>">
											<span class="bx_filter_input_checkbox">
												<?if($arItem["CODE"]=="AVIABILITY"){
                                                    $ar["VALUE"] = COption::GetOptionString("aspro.mshop", $ar["VALUE"], GetMessage($ar["VALUE"]), SITE_ID);
                                                }?>
												<span class="bx_filter_param_text" title="<?=$ar["VALUE"];?>"><?=$ar["VALUE"];?><?
